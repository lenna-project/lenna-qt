...
void Widget::loadState() {
  ...
  ui->formatComboBox->setCurrentIndex(ui->formatComboBox->findText(
      settings.value("ImageFormat", "jpg").toString()));
  ui->metadataCheckBox->setChecked(
      settings.value("CopyMetaData", true).toBool());
  ui->qualityCheckBox->setChecked(
      settings.value("ChangeQuality", false).toBool());
...
QString Widget::getImageFormat() { return ui->formatLineEdit->text(); }

bool Widget::getCopyMetaData() { return ui->metadataCheckBox->isChecked(); }
