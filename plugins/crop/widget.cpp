...
void Widget::setImage(Mat image)
{
    this->image = image;
...
void Widget::updateImage()
{
    ...
    circles[2]->moveBy(scene->sceneRect().width() - 60, scene->sceneRect().height() - 60);
...
bool Widget::isDoCrop()
{
    return doCrop;
}

std::vector<Point2f> Widget::getPoints()
{
    std::vector<Point2f> points;
    for(int i = 0; i < 4; ++i){
        int x = circles[i]->scenePos().x() + 15;
        int y = circles[i]->scenePos().y() + 15;
        points.push_back(Point2f(x * image.cols / scene->sceneRect().width(),
